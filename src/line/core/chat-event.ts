export function chatEventParticipantMids(message: any): string[] {
  ...
  return raw.split('\u001e').map((value) => value.trim()).filter((value) => value.startsWith('u'))
}
 ...
 * decoded action.
 */
export function interpretChatEvent(message: any, names: Map<string, string>): string {
  const key = typeof message?.contentMetadata?.LOC_KEY === 'string'
  ...
  switch (key) {
    case 'C_GI':
      return mids.length >= 2 ? `${name(0)} 邀請 ${name(1)} 加入群組` : '有人邀請成員加入群組'
    case 'C_MJ':
