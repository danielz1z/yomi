...
test('extracts real text from a wrapped decrypted payload', () => {
  expect(sanitizeMessagePreview(JSON.stringify({ text: '到現場後通知我' }))).toBe('到現場後通知我')
...
test('nested envelope cannot bypass the shared sanitizer', () => {
  ...
  expect(sanitizeMessagePreview({ text: '這是真正的使用者訊息' })).toBe('這是真正的使用者訊息')
})
...
test('maps rich message image and safe redirect without exposing metadata', () => {
  const result = richFields({
    ...
    contentMetadata: {
      ...
      MARKUP_JSON: JSON.stringify({ actions: [{ linkUri: 'javascript:alert(1)' }] }),
    },
  })
...
test('rich message rejects non-web media and action schemes', () => {
  const result = richFields({ contentMetadata: {
    DOWNLOAD_URL: 'file:///tmp/private.jpg',
    LINE_TAG_REDIRECTOR: 'line://nv/chat',
    MARKUP_JSON: JSON.stringify({ action: { linkUri: 'javascript:alert(1)' } }),
    ALT_TEXT: '提示',
  } })
...
test('richFields interprets Flex JSON instead of returning an attachment placeholder', () => {
  const result = richFields({ contentType: 22, contentMetadata: {
    FLEX_JSON: JSON.stringify({
      ...
      footer: { type: 'button', action: { type: 'uri', label: '查看活動', uri: 'https://example.com' } },
    }),
  } })
